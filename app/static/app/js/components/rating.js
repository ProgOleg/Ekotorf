...
(function ($) {
	$.fn.addRating = function (options) {
		...
		for (var i = 1; i <= settings.max; i++) {
			var star = $('<i/>').addClass('icon-star-border').html(this.settings.icon + '').data('rating', i).appendTo(this).click(
				function () {
		...
		$(this).append('<span class="count-star"><span id="' + settings.fieldId + '">-</span><input type="text" value="/" name="' + settings.fieldId + '" data-msg-required="Пожалуйста, поставьте оценку.">');
	};
